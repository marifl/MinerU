...
- `llm-aided-config`:
    ...
    * Optional `chunk_size` (integer, default 0 = off): for documents with more titles than this, the title list is sent in consecutive chunks of at most `chunk_size` titles instead of one request. Chunks prefer to end before chapter-like headings (numbering, or a line height in the top decile); each chunk after the first receives the still-open chapter path plus the last `chunk_context` leveled titles (default 8) as fixed context. Keeps prompt size and KV-cache memory bounded for very long documents. After all chunks are leveled, dotted heading numbers (`3.2`, `3.2.1`) are used as ground truth: a chunk whose numbered titles sit a constant offset from the document-wide majority is shifted back as a whole, and numbered titles are pinned to the majority level of their depth.
