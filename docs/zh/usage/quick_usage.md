...
- `llm-aided-config`：
    ...
    * 可选参数`chunk_size`（整数，默认0表示关闭）：当文档标题数超过该值时，标题列表按不超过`chunk_size`个标题的连续批次分批请求，而不是一次性发送。批次优先在章节样式的标题（编号，或行高处于最高十分位）之前结束；第一批之后的每一批都会附带仍然打开的章节路径和最近`chunk_context`个（默认8）已分级标题作为固定上下文。用于限制超长文档的提示词长度和KV缓存占用。所有批次完成后，会以带点的标题编号（如`3.2`、`3.2.1`）作为基准：若某一批次中带编号标题的层级与全文多数层级相差一个固定偏移，则整批回移；带编号标题最终固定到其编号深度对应的多数层级。
